...
extension PDFDocument {
    ...
    /// Returns a debug description of the PDF document
    func debugDescription() -> String {
        var info = "PDF Document: \(pageCount) pages\n"
        info += "Document attributes: \(documentAttributes?.description ?? "none")\n"
        info += "Is encrypted: \(isEncrypted)\n"
        info += "Is locked: \(isLocked)\n"
        info += "Allows copying: \(allowsCopying)\n"
        info += "Allows printing: \(allowsPrinting)\n"
        
        if pageCount > 0, let page = page(at: 0) {
            info += "First page size: \(page.bounds(for: .mediaBox).size.width) x \(page.bounds(for: .mediaBox).size.height)\n"
        }
...
// MARK: - CloudKit Record Debugging
extension CKRecord {
    /// Generate a detailed debug report for a CloudKit record
    func debugReport(label: String) -> String {
        var report = """
        📦 CLOUDKIT RECORD DEBUG REPORT: \(label)
        - Record Type: \(self.recordType)
        - Record ID: \(self.recordID.recordName)
        - Zone ID: \(self.recordID.zoneID.zoneName)
        - Creation Date: \(self.creationDate?.formatted() ?? "Unknown")
        - Modification Date: \(self.modificationDate?.formatted() ?? "Unknown")
        - Fields:
        """
        
        // Add all fields and their values
        for key in self.allKeys() {
            if let value = self[key] {
                let valueDescription: String
                if let asset = value as? CKAsset {
                    valueDescription = "CKAsset: \(asset.fileURL?.path ?? "No URL")"
                } else if let reference = value as? CKRecord.Reference {
                    valueDescription = "Reference: \(reference.recordID.recordName)"
                } else {
    ...
    /// Log detailed information about a CloudKit record
    func logDiagnostics(label: String, file: String = #file, function: String = #function, line: Int = #line) {
        startLogGroup("CloudKit Record Diagnostics: \(label)", category: .cloudKit)
        
        logCloudKit("Record diagnostics for: \(label)")
        logCloudKit(debugReport(label: label))
        
        // Check for common CloudKit issues
        if self.allKeys().isEmpty {
            logCloudKit("⚠️ Warning: Record has no fields", level: .warning)
        }
        
        // Check for PDF assets
        for key in self.allKeys() {
            if let asset = self[key] as? CKAsset {
                if let url = asset.fileURL {
                    if FileManager.default.fileExists(atPath: url.path) {
                        if let attributes = try? FileManager.default.attributesOfItem(atPath: url.path),
                           let fileSize = attributes[.size] as? Int64 {
                            logCloudKit("📄 PDF Asset found: \(key)")
                            logCloudKit("  - File exists: Yes")
                            logCloudKit("  - File size: \(ByteCountFormatter.string(fromByteCount: fileSize, countStyle: .file))")
                            
                            if fileSize == 0 {
                                logCloudKit("⚠️ Warning: PDF file is empty (0 bytes)", level: .warning)
                            }
                        }
                    } else {
                        logCloudKit("❌ Error: PDF file does not exist at path: \(url.path)", level: .error)
                    }
                } else {
                    logCloudKit("❌ Error: PDF asset has no file URL", level: .error)
                }
            }
        }
        
        endLogGroup("CloudKit Record Diagnostics: \(label)", category: .cloudKit)
    }
} 